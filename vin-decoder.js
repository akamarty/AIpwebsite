...
(function (global) {
  ...
  // ---- Client-side search log (the "spreadsheet" persistence layer) ----
  // No backend/server — every visitor's log lives in their own browser via
  // localStorage, and can be exported to a real CSV file at any time.

  var LOG_KEY = "sophiapilot_search_log";
  var LOG_MAX = 200;
  ...
  function searchLogToCSV() {
    var rows = getSearchLog();
    var header = ["timestamp", "type", "query", "result"];
  ...
  function downloadSearchLogCSV() {
    var csv = searchLogToCSV();
    var blob = new Blob([csv], { type: "text/csv;charset=utf-8;" });
    var url = URL.createObjectURL(blob);
    var a = document.createElement("a");
    a.href = url;
    a.download = "sophiapilot-search-log.csv";
  ...
  global.PorscheVIN = {
    ...
    logSearch: logSearch,
    clearSearchLog: clearSearchLog,
    searchLogToCSV: searchLogToCSV,
    downloadSearchLogCSV: downloadSearchLogCSV
  };
})(window);
